...
export function ZoneSelectedFilters({
  ...
  const filtersToShow = Object.entries(filters)
    .filter(([, values]) => Array.isArray(values) && values.length > 0) as [string, string[]][]; // Use string as key type
  ...
  const handleRemove = (type: string, valueToRemove: string) => { // Use string as key type
