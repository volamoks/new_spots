...
export default function ZoneList() {
  ...
  const handleBooking = async () => {
    ...
    for (const zoneId of selectedZones) {
      await createBooking(zoneId, startDate, endDate)
    }
            ...
            {filteredZones.map((zone) => (
              <Card
                ...
                className={`cursor-pointer transition-all duration-200 hover:shadow-lg ${
                  selectedZones.includes(zone["Уникальный идентификатор"])
                    ? "border-[#D12D35] border-2 shadow-lg"
                    : "hover:border-gray-300"
                }`}
                onClick={() => toggleZoneSelectionForBooking(zone["Уникальный идентификатор"])}
              >
                <CardHeader className="bg-secondary border-b">
                  <CardTitle className="flex justify-between items-center text-lg">
                    <span>Зона №{zone.ID}</span>
                    <Badge variant={zone["Статус"] === "Свободно" ? "success" : "secondary"}>{zone["Статус"]}</Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4 pt-4">
                  <div className="flex items-center space-x-2">
                    <MapPin className="w-4 h-4 text-[#D12D35]" />
                    <span className="text-foreground">{zone["Город"]}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Store className="w-4 h-4 text-[#D12D35]" />
                    <span className="text-foreground">
                      {zone["Маркет"]} ({zone["Формат маркета"]})
                    </span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Box className="w-4 h-4 text-[#D12D35]" />
                    <span className="text-foreground">{zone["Габариты"]}</span>
                  </div>
                  <div className="flex items-center space-x-2">
                    <Tag className="w-4 h-4 text-[#D12D35]" />
                    <span className="text-foreground">{zone["Оборудование"]}</span>
                  </div>
                </CardContent>
                <CardFooter className="bg-secondary border-t">
                  <div className="w-full text-sm text-muted-foreground">
                    <p>Основная макрозона: {zone["Основная Макрозона"]}</p>
                    <p>Смежные макрозоны: {zone["Смежная макрозона"]}</p>
                  </div>
                </CardFooter>
              </Card>
            ))}
